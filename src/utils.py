...
def compareTwoGroups(h_list, l_list, feature_name):
   print 'HIGH ({}): (count:{}, median:{}, mean:{})'.format(feature_name, len(h_list), np.median(h_list), np.mean(h_list))
   print '-'*25
   print 'LOW ({}): (count:{}, median:{}, mean:{})'.format(feature_name, len(l_list), np.median(l_list), np.mean(l_list))
   print '-'*25
