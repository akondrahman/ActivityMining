...
data_parsing = ['pickle.loads', 'pickle.load', 'pickle.Unpickler', 'cPickle.loads', 'cPickle.load', 'cPickle.Unpickler', 'marshal.loads', 'marshal.load', 
...
insecure_connection = ['httplib.HTTPSConnection', 'http.client.HTTPSConnection', 'six.moves.http_client.HTTPSConnection', 'telnetlib.*', 
...
def processBody(bodies, full_df):
    ...
    for body in bodies:
        ...
        list_.append((body_ID, 'INSECURECONN',    insecure_tup[5], post_ID, createDate,  snip_cnt, score, view, comment, favs))                        
